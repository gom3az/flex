...
- [ ] Precompute `Measured` + sanitized text once at `Row`/`Tab` build time
- [ ] Use `truncate_measured` (`width.rs:179-205`) in all draw paths; no `measure`/`sanitize` inside `draw_*`
- [ ] Cache per-tab `content_width` (help), per-header target width
...
- [ ] Drop `fill_bg` full clear; rely on ratatui `Buffer` diff (clear chrome only)
- [ ] Hoist `Layout` constraints to `const`s; construct solver once per list, not per node/detail/volume bar
- [ ] Pre-build volume/meter bar strings or render via borrowed spans; intern `FILTER_PROMPT`, `%`, `muted`; reuse thread-local format buffer instead of `format!`/`to_string` per cell
- [ ] Same for `preview.rs:118-123,129,396` (`format!` per place/delete/park)
...
- [ ] Memoize `(src → file, file → size/placed)` across frames; move `png_size` after `unchanged(pane,src)` check
- [ ] Reuse candidate/fit buffer across frames; no `cache_dir.clone()` per `sync`
...
- [ ] Gate each refresh on active tab (copy `refresh_gauges` pattern)
- [ ] Throttle proc/net/bt/notify to 2–5s or on-demand via `wake_ui`; back `bt show/devices` with `BgTask` like wifi scan
- [ ] Keep 1s tick for gauges/clock only
...
- [ ] Throttle to 2–3s; skip when proc/net tab not active
- [ ] Cache `inet_inode → pid` across ticks; rescan fds only for new pids
- [ ] Write proc-stat cache only when rates requested (Waybar `sample_json` already caches at `exec/net.rs:563-576`)
...
- [ ] Cache `extract_entities` per `(notification.id, timestamp)`; byte-level `starts_with` before `to_lowercase`; reuse meta buffers
- [ ] Recompute relative time only on minute rollover
- [ ] Hoist `format!("{} {}", comm, pid)` / SSID unescape out of per-tick loop
...
- [ ] In-place update (rewrite `label/meta/volume` only, keep `Row` ids/targets) following `center::refresh_gauges`
- [ ] `Arc<str>` / `Arc<Target>` for stable labels; `std::mem::replace` + diff instead of wholesale assign
- [ ] Single focus-restore pass
...
## Phase D — Spawns, tool resolution, binary size

### OPT-10 — Multicall binary (38MB → ~3MB)
Evidence: `flex-rice/Cargo.toml:13-75` (16× `[[bin]]`), `Cargo.toml:49-52` (LTO/strip/abort already on). Measured `target/release` ≈ 38MB total (`flex-notify` 3.6M, `flex-center/clip` 2.6M, `flex-record` 1.2M …).

- [ ] Single `flex` multicall binary; dispatcher (`src/main.rs:339` re-exec) resolves `argv[0]` (`flex-power`, …) or `flex <provider>`
- [ ] Install as hardlinks/symlinks for `flex-*` names; keep `setup.sh --check` passing
- [ ] Update `Docs/project_structure.md` entry-point list + `setup.sh` BINS

Gate: `du -cb target/release/flex*` ≈ one ~3MB binary + shims; all 16 names still work.
Verify: `cargo build --release`, `setup.sh --check`, `cargo test --locked`
...
- [ ] `OnceLock<Vec<PathBuf>>` for `PATH` dirs + per-tool `OnceLock<PathBuf>` (`nmcli/bluetoothctl/wpctl/brightnessctl`); `&'static [&'static str]` argv consts; `String::from_utf8` with lossy fallback only on error
...
4. Phase D (OPT-10, OPT-11) — packaging; last (touches install + features).
